...
import Action_DeleteAttachments from '../Actions/Action_DeleteAttachments.js';

export default function Component_Attachments(param) {
    const {
        attachments,
        list,
        itemId,
    ...
    async function removeRow(event) {
        ...
        if (check) {
            await Action_DeleteAttachments({
                list,
                itemId,
                fileNames: [
                    row.dataset.name
