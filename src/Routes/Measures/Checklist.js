import { Title, Container, FoldingCube, Card, Modal, BootstrapButton, SectionStepper, LoadingSpinner, FormSection, Table, Alert } from '../../Robi/RobiUI.js'
...
export async function Checklist(param) {
    ...
    const viewTitle = Title({
        title: `Measure #${itemId} Checklist`,
    ...
    if (editMeasureData) {
        ...
        loading = FoldingCube({
            label: `Loading Measure #${itemId} checklist`,
                        ...
                        const onHoldBtn = BootstrapButton({
                            async action(event) {
                                ...
                                $(removeHoldModal.get()).on('hidden.bs.modal', event => {
                                    Route(`Measures/${itemId}/Checklist`);
                                });
